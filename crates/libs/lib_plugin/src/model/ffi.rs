...
pub(in crate::model) type FrModelInit =
    unsafe extern "C" fn(state: *const State, control: *const Control) -> c_int;

pub(in crate::model) type FrModelStep =
    unsafe extern "C" fn(state: *const State, control: *const Control, t: f64, c: *mut C) -> c_int;
